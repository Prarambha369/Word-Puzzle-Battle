...
/** Wires every home screen button. Called once on boot. */
function initHomeScreen() {
  ...
  // Theme toggle — swaps sun/moon SVG icons
  ...
  if (settingsBtn) {
    settingsBtn.addEventListener('click', () => alert('Settings coming in Phase 2!'));
  }
  ...
  if (howBtn) {
    howBtn.addEventListener('click', () => alert(
      'HOW TO PLAY\n\n' +
      '1. Tap any empty tile on the board\n' +
      '2. Choose a letter to plant\n' +
      '3. Words of 3+ letters score in any of 8 directions\n' +
      '   3 letters = 1pt  |  4 letters = 2pts  |  5+ = 3pts\n' +
      '4. First to 20 points wins!\n\n' +
      'Tip: Both players share the board — every letter you\n' +
      'plant can be used against you.'
    ));
  }
}
...
/** Starts a new game — resets state and transitions to game screen. */
function startGame() {
  scores       = { p1: 0, p2: 0 };
  currentTurn  = 'p1';
  gameOver     = false;
...
/** Builds the board data array and renders all 100 tiles. */
...
/**
 * Creates one tile DOM element.
 * @param {number} r @param {number} c @returns {HTMLElement}
 */
...
/** Opens the bottom-sheet letter picker. */
 ...
 * @param {string} letter @param {string} owner
 */
...
/** Updates a single tile's DOM element in place. */
...
/** Triggers score-flash animation on scored tiles via rAF. */
...
/**
 * Scans 8 directions from the placed cell.
 * @param {Array} board @param {number} placedRow @param {number} placedCol @param {Object} trie
...
/** Advances the turn and fires AI if needed. */
function switchTurn() {
...
/** Fires AI move asynchronously — never blocks paint. */
function triggerAI() {
  showAIThinking(true);
  setTimeout(() => {
...
function checkWin() {
  if (scores.p1 >= WIN_SCORE || scores.p2 >= WIN_SCORE) {
    showWin(scores.p1 >= WIN_SCORE ? 'YOU' : (gameMode === 'vs-ai' ? 'AI' : 'GUEST'));
    return true;
  }
  let filled = 0;
